...
interface ScanResults {
  ...
  files: FileAnalysis[];
  symbolic_analysis: {
    role_consistency_check: { consistent: boolean };
  };
}
...
const getRoleBadgeVariant = (role: string | null): "default" | "secondary" | "destructive" | "outline" => {
  if (!role) {
    return 'outline'; // Safely handle null or undefined roles
  }
    ...
    if (scan.status === 'in_progress' || scan.status === 'pending') {
        return (
            <div className="flex flex-col items-center justify-center text-center p-8 bg-gray-800/50 rounded-lg border border-gray-700">
                ...
                <p className="text-gray-400 mt-2">The analysis for <span className="font-bold text-blue-300">{scan.repository_name}</span> is currently running. Please check back shortly.</p>
            </div>
        );
    }
    ...
    if (!scan.results || 'error' in scan.results) {
        ...
        return (
            <div className="flex flex-col items-center justify-center text-center p-8 bg-red-900/20 rounded-lg border border-red-700">
                ...
                {errorMessage && (
                    <pre className="mt-4 bg-black/50 p-4 rounded-md text-sm text-red-300 overflow-x-auto w-full max-w-2xl">{errorMessage}</pre>
                )}
            </div>
        );
    }
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="bg-gray-800/50 border-gray-700"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium text-gray-300">Symbolic Role Check</CardTitle><Shield className="h-4 w-4 text-gray-400"/></CardHeader><CardContent>{results.symbolic_analysis.role_consistency_check.consistent ? (<div className="flex items-center text-green-400"><CheckCircle className="h-6 w-6 mr-2" /><span className="text-2xl font-bold">Passed</span></div>) : (<div className="flex items-center text-red-400"><XCircle className="h-6 w-6 mr-2" /><span className="text-2xl font-bold">Failed</span></div>)}<p className="text-xs text-gray-400">Z3 SMT solver consistency result</p></CardContent></Card>
        </div>
