    ...
    useEffect(() => {
        ...
        class Stream {
            ...
            init() {
                this.alpha = 0;
                this.isDying = false;
                const directions: Stream['direction'][] = ['up', 'down', 'in'];
                this.direction =
                ...
                this.speed =
                    (Math.random() * 0.3 + 0.15) * (this.fontSize / 15);
                ...
                switch (this.direction) {
                    ...
                    case 'in': {
                        ...
                        const inDx = vanishingPointX - this.x;
                        const inDy = vanishingPointY - this.y;
                        const inDist = Math.sqrt(inDx * inDx + inDy * inDy) || 1;
                        this.vx = (inDx / inDist) * Math.abs(this.speed) * 15;
                        this.vy = (inDy / inDist) * Math.abs(this.speed) * 15;
                        break;
                    }
                }
            ...
            updateAndDraw() {
                ...
                if (this.isDying) {
                    if (this.symbols.length > 0 && Math.random() > 0.9) {
                        this.symbols.pop();
                ...
                switch (this.direction) {
                    case 'down':
                        this.y += this.speed;
                        hasReachedEnd =
                            this.y - this.length * this.fontSize > height;
                        break;
                    case 'up':
                        this.y -= this.speed;
                        hasReachedEnd =
                ...
                this.symbols.forEach((symbol, index) => {
                    ...
                    if (this.isDying) {
                        symbolAlpha = Math.max(
                            0,
                            symbolAlpha -
                                this.alpha * (index / this.symbols.length)
                        );
                    } else {
                        symbolAlpha *= this.alpha;
                    }

                    if (isHead) {
                        ctx.fillStyle = `rgba(229, 231, 235, ${this.alpha})`;
                    } else {
                        const finalTailAlpha = symbolAlpha * 0.5 + 0.05;
